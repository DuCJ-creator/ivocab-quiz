...
export default function Home() {
  ...
  const generateQuiz = async (targetMode: 'online' | 'paper') => {
    ...
    const unitWords = fullData.filter(w =>
      selectedUnits.includes(parseInt(w.unit))
    );
  ...
  const calculateScore = () =>
    quizData.reduce((acc, q, idx) => acc + (userAnswers[idx] === q.word ? 1 : 0), 0);
        ...
        {step === 'setup' && (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 md:p-12">
                ...
                {[1, 2, 3, 4, 5, 6].map(lvl => (
                  <button
                    ...
                    className={`px-6 py-2 rounded-md font-medium border ${
                      selectedLevel === lvl ? 'bg-slate-800 text-white' : 'bg-white text-slate-600'
                    }`}
                  >
                    Level {lvl}
                  </button>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {availableUnits.map(unit => (
                      <button
                        ...
                        className={`w-12 h-12 rounded-md font-bold text-sm ${
              ...
              <div className="grid grid-cols-4 gap-4">
                {[5, 10, 20, 30].map(count => (
                  <button
                    ...
                    className={`py-3 rounded-md border font-medium ${
                      questionCount === count
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <button
                onClick={() => generateQuiz('online')}
                disabled={selectedUnits.length === 0}
                className="w-full py-4 bg-slate-900 text-white rounded-lg font-bold text-lg hover:bg-slate-800 shadow-lg flex justify-center items-center gap-2"
              >
                <MonitorPlay className="w-5 h-5" /> Start Online Quiz
              </button>
              <button
                onClick={() => generateQuiz('paper')}
                disabled={selectedUnits.length === 0}
                className="w-full py-4 bg-white border-2 border-slate-900 text-slate-900 rounded-lg font-bold text-lg hover:bg-slate-50 shadow-sm flex justify-center items-center gap-2"
              >
        ...
        {step === 'generating' && (
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-serif font-bold">
              Generating {mode === 'paper' ? 'Paper Quiz' : 'Questions'}...
            </h3>
              ...
              <div className="grid gap-3">
                {quizData[currentQIndex].options.map((opt: string, idx: number) => {
                  ...
                  if (userAnswers[currentQIndex]) {
                    if (userAnswers[currentQIndex] === opt && opt === quizData[currentQIndex].word)
                      style = "border-teal-500 bg-teal-50 text-teal-800 ring-1 ring-teal-500";
                    else if (userAnswers[currentQIndex] === opt)
                      style = "border-red-500 bg-red-50 text-red-800 ring-1 ring-red-500";
                    else if (opt === quizData[currentQIndex].word)
                      style = "border-teal-500 bg-teal-50 text-teal-800 opacity-70";
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 rounded-lg border-2 text-left flex justify-between items-center ${style}`}
                    >
          ...
          <div className="bg-white max-w-4xl mx-auto">
                    ...
                    <div className="flex gap-4 mb-4">
                      ...
                      <div className="flex-1">
                        <p className="text-slate-800 leading-relaxed font-serif text-lg">
                          {q.sentence.split('______').map((p: string, i: number) => (
                            <span key={i}>
                              {p}
                              {i === 0 && (
                                <span className={`font-bold px-2 mx-1 border-b-2 ${isCorrect ? 'border-teal-500 text-teal-800' : 'border-red-500 text-red-700'}`}>
                                  {userAnswers[idx] || '(No Ans)'}
                                </span>
                              )}
                            </span>
                          ))}
            ...
            <div className="mt-12 flex justify-center gap-4 print:hidden pb-12">
              ...
              <button
                ...
                className="px-8 py-3 rounded-lg bg-teal-600 text-white shadow-lg hover:bg-teal-700 flex gap-2"
              >
            ...
            <div className="print:hidden mb-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex items-center justify-between">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-4 py-2 rounded bg-white border border-slate-300 text-sm font-bold hover:bg-slate-50 flex items-center gap-2"
                >
                  <ArrowLeft className="w-4 h-4" /> Back
                </button>
                <button
                  onClick={() => window.print()}
                  className="px-4 py-2 rounded bg-slate-900 text-white text-sm font-bold hover:bg-slate-700 flex items-center gap-2"
                >
